        ...
        try {
            ...
            if(id!=null){
                ...
                for (Community community : communities) {//获取小区边界，平均价格，房源套数
                    ...
                    c_prices.add(communityDao.FindSecondAVGPriceByCommunity(community.id));
                    c_nums.add(communityDao.FindSecondNumByC_id(community.id));
                }
            }
       ...
       %>
        ...
        var AddAdminOverlays = function () {
            ...
            var OnClick = function (){//点击查看次行政区下的小区
                window.location.href="SecondMap.jsp?id=<%=adminAreas.get(i).id%>";
            }
            var myOverlay = new MyOverlay(point,"<%=adminAreas.get(i).name%>","<%=nums.get(i)%>套","<%=prices.get(i)%>元/平",
        ...
        var AddCommunityOverlays = function () {
            ...
            %>
                ...
                if (map.getDistance(point, map.getCenter()) <= 800) {//只渲染部分小区
                    ...
                    var myOverlay = new MyOverlay(point,"<%=communities.get(i).name%>","<%=c_nums.get(i)%>套","<%=c_prices.get(i)%>元/平",
                        MouseOverListener, MouseOut,OnClick);
                    //添加覆盖物
                    map.addOverlay(myOverlay);
                }
