...
<html>
  ...
  <form method="get" action="FindSecHouse">
    <table style="background-color: #f8f3f3;margin-left: 15%;padding: 10px">
      ...
      <tr>
        <td></td>
        <td style="text-align: right"><input style="width: 60px;height: 30px" type="submit" name="FindSecondHouse" value="查找"></td>
      </tr>
    </table>
  </form>
  <br><br>
  ...
  <%
    ...
    try {
      if(request.getAttribute("houseList")==null) {//条件查询
        ...
        SecondNum = houseDao.FindSecondNum(adminArea, null, minprice, maxprice, H_type, minarea, maxarea);
      }
      else{//根据周边查询
        houseList=(List<Second_House>)request.getAttribute("houseList");
        SecondNum=houseList.size();
  ...
  <%
    //页面个数
    int PageCount=SecondNum%PageSize==0?SecondNum/PageSize:SecondNum/PageSize+1;
    if(houseList.size()==0){
  %>
  ...
  %>
        <div style="background-color: #ffffff;">
            ...
            <span style="display: inline-block;">
              <a style="font-weight:bold;color: black" href="SecHouseDetail.jsp?H_id=<%=houseList.get(i).id%>">点击查看详情</a>
              <br>
              <%=houseList.get(i).area%>平方米/<%=houseList.get(i).direct%>/<%=houseList.get(i).type%>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              <span style="color: orange;font-size: x-large;vertical-align: bottom" ><%=houseList.get(i).price%>元/平</span>
            </span>
            <hr>
        </div>
  ...
  %>
        <div style="margin-right: 0;text-align: right">
          ...
          <br>
          <a style="color: black" href="FindSecHouse?ShowPage=0<%=condition%>">首页</a>
          <%
            if(Integer.parseInt(ShowPage)>0){
          %>
          <a style="color: black" href="FindSecHouse?ShowPage=<%=Integer.parseInt(ShowPage)-1%><%=condition%>">上一页</a>
          <%
            }
          %>
          <%   //根据PageCount的值显示上下2页的数字并附加上相应的超链接
            for(int i=Integer.parseInt(ShowPage)-2;i<=Integer.parseInt(ShowPage)+2;i++){
              if(i>=0&&i<PageCount){
          %>
          <a style="color: black" href="FindSecHouse?ShowPage=<%=i%><%=condition%>"><%=i+1%></a>
          <%
              }
            }
          %>
          ...
          %>
          <a style="color: black" href="FindSecHouse?ShowPage=<%=Integer.parseInt(ShowPage)+1%><%=condition%>">下一页</a>
          <%
            }
          %>
          <a style="color: black" href="FindSecHouse?ShowPage=<%=PageCount-1%><%=condition%>">末页</a>
        </div>
      </div>
      <div style="background-color: #ffffff;display: inline-block;vertical-align: top;">
        ...
        <span style="display: block;margin-left: 15px">
          <input style="width: 100%;height: 60px" type="submit" onclick='location.href=("SecondMap2.jsp")'
                 value="试试地图找房">
        </span>
      </div>
    </div>
